...
class BreathingCircleView : View {
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        // circles as well as arcs are drawn 50% outside of the bounds defined by the radius
        // (radius for arcs is calculated from the rectangle mRect).
        // So if mRect is too large, the lines will not fit into the View

        //val totalSecs = inhaleSecs + exhaleSsecs + hold1Secs + hold2Secs
        //val sweepAngle = (inhaleSecs / totalSecs) * (180 / PI).toFloat()
...
/*public void drawArc(RectF oval,
                    ...
                    boolean useCenter,
                    Paint paint)
The drawArc method requires a RectF object which you can think of as the rectangle that
encloses the area where you want to draw the arc; also, you need to specify the startAngle,
the sweepAngle, the useCenter if you want to fill the arc or not and the paint object.


set the anti alias in order to have smooth edges

A small note around the START_ANGLE: when drawing an arc on a canvas,
if the start angle is 0f the arc will start at 3 o’clock on a watch but for this specific custom view
I want it to start at 10:45 o’clock instead, so adding
private const val START_ANGLE = 225f
to the startAngle it will actually move the starting point to the right place
because the drawArc method calculates the start angle as a modulo of 360.
